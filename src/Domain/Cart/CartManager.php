...
class CartManager
{
    ...
    private function stringedOptionsValues(array $optionValues = []): string
    {
        sort($optionValues);
    ...
    public function add(Product $product, int $quantity = 1, array $optionValues = []): Model|Builder
    {
        $cart = Cart::query()
            ...
            ], $this->storedData($this->cartIdentityStorage->get()));


        $cartItem = $cart->cartItems()
            ->updateOrCreate([
                'product_id' => $product->getKey(),
                'string_option_values' => $this->stringedOptionsValues($optionValues)
            ], [
                'price' => $product->price,
